...
def process_serial_line(line):
    ...
    try:
        ...
        roll, pitch, yaw = map(np.deg2rad, map(float, parts[4:7]))
        return {
            'timestamp': timestamp,
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # ws and ad for z and rotate movement
        ...
        if USE_SERIAL:
            ...
            serial_data = process_serial_line(line)
            if serial_data:
                ax, ay, az = serial_data['accel']
                roll, pitch, yaw = serial_data['euler']
                ...
                # find delta_quat = curr_imu_quat * inverse(reference_quat)
                quat = mul_quat(curr_imu_quat, [reference_quat[0], -reference_quat[1], -reference_quat[2], -reference_quat[3]])
        ...
        for (axis_vec, color, neg_color, label) in sorted_axes:
            draw_axis(screen, quat, axis_vec, color, neg_color, label, font_s)
        ...
        for i in range(3):
            ...
            draw_bar(screen, y, gyro_vals[i],  10.0, GYRO_COLORS[i],  ["GX","GY","GZ"][i], font_s)

        for i in range(3):
            y = 136 + i * (BAR_H + BAR_GAP)
            draw_bar(screen, y, accel_vals[i], 20.0, ACCEL_COLORS[i], ["AX","AY","AZ"][i], font_s)

        accel_offset = [ax_w, ay_w, az_w]
        for i in range(3):
            y = 136 + BAR_H*3 + BAR_GAP*3 + i * (BAR_H + BAR_GAP) + TEXT_PADDING
            draw_bar(screen, y, accel_offset[i], 20.0, (80,80,80), ["OFFX","OFFY","OFFZ"][i], font_s)
        ...
        # rx rotates x and z around y
        rot_rate = math.radians(30) # rad/s
        rx *= rot_rate * DT
        new_x = position[0] * math.cos(rx) - position[2] * math.sin(rx)
        new_z = position[0] * math.sin(rx) + position[2] * math.cos(rx)
        position[0] = new_x
        position[2] = new_z
